"""Command-line entry for Agent A.

Usage:
    python -m agents.reproducibility <sample-dir> [--dry-run] [--budget 8]

Runs Agent A against a sample directory (e.g. `demo-inputs/s1-cve-2026-3849/`)
and prints a one-line verdict plus the path to the findings directory.
"""
...
from agents.reproducibility.agent import (
    DEFAULT_BUDGET_USD,
    DEFAULT_EFFORT,
    DEFAULT_MAX_TURNS,
    DEFAULT_MODEL,
...
# Load .env at import; does not override existing shell env vars.
load_dotenv()


def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="agents.reproducibility",
        description="Run Agent A (Reproducibility Verifier) on a recorded sample.",
    )
...
async def _run(args: argparse.Namespace) -> int:
    ...
    result = await run_agent_a(**kwargs)
    ...
    if result.artifact is not None:
        ...
        print("        verdict: (no A_reproducibility.json written — sub-agent did not finish)")
    print(
